module.exports = {
  ...
  GetOutgoingRels: function (uuids, SubGraph, label, db, onSuccess, onError) {
    db.cypher({ query: "match (from_node)-[rel:" + label + "]->(to_node) where from_node.uuid in {uuids} return from_node, rel, to_node", params: { uuids } },
      (err, results) => {
        if (err && onError) onError(err);
        results.map((relationship) => {
          this.MergeRemoteRelationship(relationship, SubGraph);
        })
        onSuccess(results);
      });
  },

  GetIncomingRels: function (uuids, SubGraph, label, db, onSuccess, onError) {
    db.cypher({ query: "match (from_node)-[rel:" + label + "]->(to_node) where to_node.uuid in {uuids} return from_node, rel, to_node", params: { uuids } },
      (err, results) => {
        if (err && onError) onError(err);
        results.map((relationship) => {
          this.MergeRemoteRelationship(relationship, SubGraph);
  ...
  AddRemoteNodeToSubgraph: function (uuid, SubGraph, db, onSuccess, onError) {
    if (!uuid) onError();
    db.cypher({ query: `match (node {uuid:'${uuid}'}) return node` },
      (err, results) => {
        if (err || results.length == 0 || !results[0].node) { if(onError) onError(); return; }
        var result = results[0].node;
        delete result._id;
        if (SubGraph) this.MergeNode(result, SubGraph);
        onSuccess(result)
      })
  },
